...
class CausalCrossConditionalLinearAttention(nn.Module):
    """
    Args:
    ...
    def __init__(self, 
        ...
        super(CausalCrossConditionalLinearAttention, self).__init__()
        ...
        self.n_head = self.num_heads  # Alias for number of heads
        ...
        self.resid_drop = nn.Dropout(resid_drop)  # Residual dropout
    ...
    def forward(self, x):
        ...
        att = torch.matmul(q, k.transpose(-2, -1)) * (1.0 / math.sqrt(k.size(-1)))  # Scaled dot-product attention
        # Apply causal mask to ensure no peeking at future positions (upper triangle mask)
        att = att.masked_fill(self.mask[:,:,:t,:t].repeat(1, 1, 3, 3) == 0, float('-inf'))  # Apply the causal mask
