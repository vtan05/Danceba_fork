...
sys.path.append(os.path.abspath('/data15/chenjh2309/Train/Dancing-main/ori2/utils'))
...
def img2video(expdir, epoch, audio_path=None):
    ...
    audio_dir = "/data15/chenjh2309/Train/Dancing-main/aist_plusplus_final/all_musics"
...
def visualize_json(fname_iter, image_dir, dance_name, dance_path, config, quant=None):
    ...
    img = img.transpose(Image.FLIP_TOP_BOTTOM)  # 翻转图像
    img = np.asarray(img)  # 转换为 NumPy 数组

    # 确保 img 是可写的
    img = img.copy()  # 添加这一行以确保 img 是可写的

    # 添加文本到图像
    if quant is not None:
        cv2.putText(img, str(quant[j]), (config.width-400, 80), cv2.FONT_HERSHEY_SIMPLEX, 1, (0,255,0), 3)

    img = cv2.cvtColor(img, cv2.COLOR_BGR2BGRA)  # 转换颜色通道
    img = Image.fromarray(np.uint8(img))  # 转回图像
    img.save(os.path.join(f'{image_dir}/{dance_name}', f'frame{j:06d}.png'))  # 保存图像
...
def main():
    config = {'height': 540, 'width': 960, 'ckpt_epoch': 10}

    dance_names = "gBR_sBM_cAll_d04_mBR0_ch01"
    write2json(np_dances, dance_names, config, expdir, epoch)
    pkl_root = "/data15/chenjh2309/Train/Dancing-main/ori2/experiments/cc_motion_gpt/vis"
    visualize(config, dance_names, pkl_root, 123221, quants=None)
    img2video(pkl_root,12321)
