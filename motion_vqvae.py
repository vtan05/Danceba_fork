...
class MoQ():
    ...
    def __init__(self, args):
        self.config = args
        self._build_test_loader() # 3/14/23.41 代码缺少test_loader（存在于_build_中）的类，
        torch.backends.cudnn.benchmark = True
        try:
            self._build()
        except Exception as e:
            print("Error occurred during MoQ initialization:", e)
    ...
    def _build(self):
        ...
        try:
        # 直接指定配置文件的路径
            config_file_path = '/home/gisp3/Additional_Disk_4T_1/fcy/dancing/Bailando/data/aistpp_train_wav'
            with open(config_file_path) as f:
                config_data = json.load(f)
        except Exception as e:
            print("Error occurred during loading config file:", e)
